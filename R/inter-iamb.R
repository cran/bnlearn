...
inter.incremental.association = function(data, cluster = NULL, whitelist,
  ...
  mb = smartSapply(cluster, as.list(nodes), inter.ia.markov.blanket,
...
inter.ia.markov.blanket = function(target, data, nodes, alpha,
  ...
  repeat {
    ...
    association = indep.test(nodes[nodes %!in% c(mb, culprit)], target, sx = mb,
