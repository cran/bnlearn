...
SEXP result, try, undirected;
  ...
  /* preallocate an array for the variable types, to be used to avoid
  ...
  /* check variable types in each arc are ok. */
  for (i = 0; i < narcs; i++) {
...
SEXP split, dpar, gpar, arcs;
  ...
  /* cache the variable types from data frames and fitted networks. */
