...
SEXP onLoad(void) {
  ...
  BN_CutpointsSymbol = install("cutpoints");
...
static const R_CallMethodDef CallEntries[] = {
  ...
  CALL_ENTRY(completely_directed, 2),
  CALL_ENTRY(configurations, 3),
  CALL_ENTRY(connected_components, 2),
  ...
  CALL_ENTRY(naivepred, 6),
  CALL_ENTRY(normalize_cpt, 1),
  CALL_ENTRY(nparams_structure, 4),
  CALL_ENTRY(nparams_fitted, 2),
  ...
  CALL_ENTRY(predict_from_parents, 5),
  CALL_ENTRY(predict_map_lw, 7),
  ...
  CALL_ENTRY(zero_inflated_hyperpoisson_parameters, 10),
  CALL_ENTRY(zero_inflated_negative_binomial_parameters, 10),
  {NULL, NULL, 0}
};
